...
export namespace Glob {
  ...
  export function findMatchPaths(obj: any, pattern: string): string[] {
    if (!obj || !pattern) {
      return [];
    }
      ...
      if (curKey === ALL) {
        ...
        return flatten(
          Object.keys(curValue).map(key => {
            ...
            return findMatchPaths(curValue[key], `${nextPaths.join(DIVIDER)}`).map(p =>
              concatPath(parentPath, key, p),
            );
          }),
        );
      }
