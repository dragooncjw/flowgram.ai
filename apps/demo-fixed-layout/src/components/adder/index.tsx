...
import { ClipboardService, FlowOperationService } from '@flowgram.ai/fixed-layout-editor';
import { type FlowNodeEntity, usePlayground, useService } from '@flowgram.ai/fixed-layout-editor';
...
export default function Adder(props: {
  ...
  const flowOperationService = useService<FlowOperationService>(FlowOperationService);
  ...
  const handlePaste = useCallback(async (e: any) => {
    try {
      ...
      nodes.reverse().forEach((n: FlowNodeEntity) => {
        ...
        flowOperationService.addFromNode(from, newNodeData);
      });
  ...
  return (
    <Popover
      ...
      content={<Nodes onSelect={add} from={from} />}
      placement="right"
      trigger="click"
